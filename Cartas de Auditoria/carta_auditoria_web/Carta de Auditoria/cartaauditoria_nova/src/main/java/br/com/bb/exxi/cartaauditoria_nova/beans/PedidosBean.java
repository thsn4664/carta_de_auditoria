...
import br.com.bb.exxi.cartaauditoria_nova.entidades.VwResultadosPK;
import br.com.bb.exxi.cartaauditoria_nova.jdbc_base.ConsultaAPI;
import br.com.bb.exxi.cartaauditoria_nova.jdbc_base.ConsultaDB2;
...
//    private List<Pedidos> pedidosAProcessar;
    ...
    @PostConstruct
    public void init() {
        System.out.println("PedidosBean init");
...
//        pedidosAProcessar = new ArrayList<Pedidos>();
    ...
    public void atualizar() {
        System.out.println(" inicio atualizar, chama: listaPedidos,montaListaDtBase teste se funDep'9951'=acessoFull");
        controlaVisao();
        if (usuario != null) {
//            pedidosAProcessar = pedidosDAO.listaPedidosAProcessar(Integer.parseInt(usuario.getCodigoPrefixoDependencia()));
        ...
        System.out.println(" fim atualizar, chama: listaPedidos,montaListaDtBase teste se funDep'9951'=acessoFull");
    }
...
//                *** SÓ PARA TESTE ***                
                if(funci.getMatricula().equals("F8711907") || funci.getMatricula().equals("F8315465") || funci.getMatricula().equals("F5514319") || funci.getMatricula().equals("F6770681") ){
                    okEstatistica = true;
                    okAcesso = true;
                }
...
//            arqExcel = Workbook.createWorkbook(externalContext.getResponseOutputStream());
            ...
            //WritableImage image = new WritableImage(1, 2, 2, 2, new File("C:\\Users\\F5514319\\OneDrive - Banco do Brasil S.A\\Documentos\\NetBeansProjects\\cartaauditoria_nova\\src\\main\\resources\\img\\LogoBB.png")); //local
            image.setImageAnchor(WritableImage.NO_MOVE_OR_SIZE_WITH_CELLS); 
            s.addImage(image);
            Locale localeBR = new Locale("pt", "BR");
            s.addCell(new Label(1, 4, "São Paulo, " + new SimpleDateFormat("dd 'de' MMMM 'de' yyyy", localeBR).format(new Date()), normalStyleRight));
...
/*inicio alteracao BB CDB TMS*/

/*teste se existe na lista da ditec*/
int chaveadorCDBtms=0;  

for (VwResultados vr : listaResultados) {
    if (vr.getNmMdld().equals("BB CDB TMS")  ) {  
        chaveadorCDBtms=1;
    }
} 

if (chaveadorCDBtms == 1 ) {
    
    List<String> listaOpeCDBtms = new ArrayList<String>();
    
    for (VwResultados vr : listaResultados) {
        if (vr.getNmMdld().equals("BB CDB TMS")  ) { 
            
              listaOpeCDBtms.add(vr.getVwResultadosPK().getNmCtr()); // inclui na lista para posterior verificacao
              
    //        System.out.println("vr.getVwResultadosPK()"+vr.getVwResultadosPK());
    //        System.out.println("vr.getNmMdld()"+vr.getNmMdld());
    //        System.out.println("vr.getVlSld1()"+vr.getVlSld1());
    //        System.out.println("vr.getVlSld1()"+vr.getVlSld1());
    //        System.out.println("vr.getVlSld2()"+vr.getVlSld2());
    //        System.out.println("vr.getVlSld3()"+vr.getVlSld3());
    //        System.out.println("vr.getVlSld4()"+vr.getVlSld4());
    //        System.out.println("vr.getCdColSld()"+vr.getCdColSld());
        }
    }
    
    /*inicio exclui BB CDB TMS existente*/
        Iterator<VwResultados> it = listaResultados.iterator();
        while (it.hasNext()) {
            VwResultados vrs = it.next();
            if ("BB CDB TMS".equals(vrs.getNmMdld())) {
                it.remove(); // Remove com segurança
            }
        }
    /*fim exclui BB CDB TMS existente*/

    String dataInicioPesquisa="01."+(new SimpleDateFormat("MM.yyyy").format(p.getPedidosPK().getDtBase()));
    String dataFimPesquisa=new SimpleDateFormat("dd.MM.yyyy").format(p.getPedidosPK().getDtBase());
    //System.out.println("cnpj"+cnpj);

    ConsultaAPI api = new ConsultaAPI();
    //String stringSwaggerDadosMCI = api.postAPI("https://gs4935.intranet.bb.com.br/api/dados", ConstantsSSO.ssoToken, "{\"mci\": \""+cdCli+"\"}");
    //JSONObject objServerResponse = new JSONObject(stringSwaggerDadosMCI);
    //JSONObject data = objServerResponse.getJSONObject("data"); 
    //JSONObject dataObs = data.getJSONObject("data"); 
    //JSONArray arrayDados = dataObs.getJSONArray("contasCorrentes");
    
    String stringSwaggerDadosMCI = api.postAPI("https://gs4935.intranet.bb.com.br/api/contasCorrenteCliente", ConstantsSSO.ssoToken, "{\"mci\": \""+cdCli+"\"}");
    
    JSONObject objServerResponse = new JSONObject(stringSwaggerDadosMCI);
    JSONObject data = objServerResponse.getJSONObject("data"); 
    JSONObject dataObs = data.getJSONObject("data"); 
    JSONArray arrayDados = dataObs.getJSONArray("listaTabelaContaCorrente");
    for (int i = 0; i < arrayDados.length(); i++) {
        JSONObject objContas = arrayDados.getJSONObject(i);
        //System.out.println("order"+i);
        //System.out.println("titularidade"+objContas.getInt("titularidade"));
        //System.out.println("prefAgencia"+objContas.getInt("prefAgencia"));
        //System.out.println("contaCorrente"+Integer.parseInt(objContas.getString("contaCorrente")));

        
        System.out.println("objContas.getInt(\"codigoDependenciaVinculadoCliente\")"+objContas.getInt("codigoDependenciaVinculadoCliente"));
        System.out.println("objContas.getInt(\"numeroContaCorrenteVinculadoCliente\")"+objContas.getInt("numeroContaCorrenteVinculadoCliente"));
        String operacoesPlataformaMCI = api.postAPI("https://plataforma.atendimento.bb.com.br:49286/cac-invest-api/v3/api/cac/Op6387132-v2", ConstantsSSO.ssoToken, "{\"codigoCliente\":"+cdCli+",\"codigoProduto\":1,\"codigoModalidade\":50,\"dataInicioPesquisa\":\""+dataInicioPesquisa+"\",\"dataFimPesquisa\":\""+dataFimPesquisa+"\",\"codigoPrefixoDependencia\":"+objContas.getInt("codigoDependenciaVinculadoCliente")+",\"numeroContaCorrente\":"+objContas.getInt("numeroContaCorrenteVinculadoCliente")+"}");
        System.out.println("operacoesPlataformaMCI"+operacoesPlataformaMCI);
        
        
        JSONObject objoperacoesPlatMCI = new JSONObject(operacoesPlataformaMCI);
        JSONObject objoperacoesPlatMCIdata = objoperacoesPlatMCI.getJSONObject("data");
        if (objoperacoesPlatMCIdata.has("listaOcorrencia") && objoperacoesPlatMCIdata.get("listaOcorrencia") instanceof JSONArray) {
            
            JSONArray arrayListocorrencia = objoperacoesPlatMCIdata.getJSONArray("listaOcorrencia");
            for (int j = 0; j < arrayListocorrencia.length(); j++) {
                JSONObject objOperacoes = arrayListocorrencia.getJSONObject(j);

                if((objOperacoes.getDouble("listaValorSaldoCapital")+objOperacoes.getDouble("listaValorRendimentoOperacao")) > 0 ){
                    //System.out.println("nome modalidade"+"BB CDB TMS");
                    //System.out.println("prefAgencia"+objContas.getInt("prefAgencia"));
                    //System.out.println("listaNumeroOperacao"+objOperacoes.getLong("listaNumeroOperacao"));

                    double valor1 = objOperacoes.getDouble("listaValorSaldoCapital");
                    double valor2 = objOperacoes.getDouble("listaValorRendimentoOperacao");

                    //Convertido para bigDecimal por ser mais exato
                    //BigDecimal bd = new BigDecimal(bdvalor2 + bdvalor1).setScale(2, RoundingMode.HALF_UP); // arredonda para o mais perto
                    //BigDecimal bd = new BigDecimal(valor1 + valor2).setScale(2, RoundingMode.CEILING); //arredonda para cima

                    BigDecimal bdvalor1 = new BigDecimal(valor1).setScale(2, RoundingMode.HALF_UP);
                    BigDecimal bdvalor2 = new BigDecimal(valor2).setScale(2, RoundingMode.HALF_UP);

                    BigDecimal bd = bdvalor1.add(bdvalor2);

                    double saldoBruto = bd.doubleValue();
                    //System.out.println("Saldo Brunto"+saldoBruto);

                    VwResultadosPK novoResultadosPK = new VwResultadosPK();                    
                    novoResultadosPK.setDtBase(null);            
                    novoResultadosPK.setCdCli(null);            
                    novoResultadosPK.setCdPrd( 1 );
                    novoResultadosPK.setCdMdld(50); 
                    novoResultadosPK.setCdDepe(objContas.getInt("codigoDependenciaVinculadoCliente"));
                    novoResultadosPK.setNmCtr(String.valueOf(objOperacoes.getLong("listaNumeroOperacao")));            

                    VwResultados novoSaldo = new VwResultados();
                    novoSaldo.setNmMdld("BB CDB TMS");
                    novoSaldo.setVlSld1(saldoBruto);
                    novoSaldo.setVlSld2(0.0);
                    novoSaldo.setVlSld3(0.0);
                    novoSaldo.setVlSld4(null);
                    novoSaldo.setCdColSld("1");
                    novoSaldo.setVwResultadosPK(novoResultadosPK);
                    
                    //TRAVA OPERACOES NA LISTA BBM
                    //System.out.println("listaOpeCDBtms.toString()"+listaOpeCDBtms.toString());
                    //System.out.println("String.valueOf( objOperacoes.getLong(\"listaNumeroOperacao\") )"+String.valueOf( objOperacoes.getLong("listaNumeroOperacao") ) );
                    //if( listaOpeCDBtms.toString().contains(String.valueOf( objOperacoes.getLong("listaNumeroOperacao") )) ){
                        listaResultados.add(novoSaldo);
                    //} 
                } 
            }
        }
    ...
    /*verifica se produto incluido*/
    //System.out.println("verifica se produto incluido");
    //for (VwResultados vr : listaResultados) {
    //    if (vr.getNmMdld().equals("BB CDB TMS")  ) {
    //        System.out.println("vr.getVwResultadosPK()"+vr.getVwResultadosPK());        
    //        System.out.println("vr.getNmMdld()"+vr.getNmMdld());
...
//                                String aux = rs2.getString("vl_encargos");
                                    ...
                                    /* tratamento ACC */
                                    double aux2 = 0.0;
                                    if(vr.getVlSld1() != 0 & vr.getVlSld2() != 0){
            ...
            s.addCell(new Label(1, lin + 1, nmDep.toUpperCase().replace("CEAD/SOLUCOES EM AUTOMACAO","Centro ADM/SOLUCOES EM AUTOMACAO"), rodapeStyleBoldCenter));
...
//            verticalStyle11.setBorder(Border.ALL, BorderLineStyle.THIN);
            ...
            //WritableImage image = new WritableImage(2, 1, 2, 2, new File("C:\\Users\\F5514319\\OneDrive - Banco do Brasil S.A\\Documentos\\NetBeansProjects\\cartaauditoria_nova\\src\\main\\resources\\img\\correios-logo-5.png"));//local
            image.setImageAnchor(WritableImage.NO_MOVE_OR_SIZE_WITH_CELLS); 
            s.addImage(image);
            ...
            if(enderecoDep != null){
                s.addCell(new Label(2, 14, String.format("%04d", enderecoDep.getCdPrefDep())+"-"+enderecoDep.getDvPrefDep()+" "+enderecoDep.getNmDep().replace("CEAD/SOLUCOES EM AUTOMACAO","Centro ADM/SOLUCOES EM AUTOMACAO"), normalStyle11));
